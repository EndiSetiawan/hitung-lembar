...
class _HitungKasPageState extends State<HitungKasPage> {
  final Map<int, int> noteCount = {
    for (final n in notes) n.value: 0,
  };
  ...
  void pressNumber(int number) {
    ...
    setState(() {
      ...
      if (activeNote != null) {
        noteCount[activeNote!] = v;
      }

      if (activeCoin != null) {
        coinCount[activeCoin!] = v;
      }
    });
  }
  ...
  Future<void> resetAll() async {
    final yes = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'Semua jumlah uang akan dikembalikan ke nol.',
        ),
        actions: [
          TextButton(
  ...
  String receiptText() {
    ...
    final no =
        'STRUK-${DateFormat('yyyyMMdd').format(now)}-${1000 + now.millisecond}';
    ...
    for (final n in notes) {
      ...
      if (q > 0) {
        ...
        b.writeln(
          '${rupiah(n.value)} x $q lbr = ${rupiah(n.value * q)}',
        );
      }
    }
  ...
  Future<Uint8List?> captureReceipt() async {
    final boundary = receiptKey.currentContext
  ...
  Future<void> downloadJpgLike() async {
    ...
    final file = File(
      '${dir.path}/Struk_Kas_${DateTime.now().millisecondsSinceEpoch}.png',
    );
    ...
    await Share.shareXFiles(
      [XFile(file.path)],
      text: 'Struk perhitungan kas ${rupiah(total)}',
    );
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Text(
                              'TGL : ${DateFormat('dd/MM/yyyy').format(now)}',
                            ),
                            Text(
                        ...
                        // FIX:
                        // Tidak boleh const karena DateFormat().format()
                        // adalah pemanggilan method runtime.
                        Text(
                          'NO  : STRUK-${DateFormat('yyyyMMdd').format(now)}',
                        ),
